...
class TestUpdateBook(BaseTest):
    ...
    def create_book_before_update_book_test(self, init_api_client):
        ...
        book = {"title": "PUT API Test Book Title",
    ...
    def update_and_validate(self, update, expected_status=200, expected_message=None, book_id=None):
    ...
    def test_should_return_401_when_no_auth_is_provided_on_update_book(self):
        ...
        response = self.client.put(
            self.client.build_url(f"/{self.book_id}"), json={"author": "test 401 author"})
    ...
    def test_should_return_404_when_book_with_id_is_not_exists(self):
        ...
        self.update_and_validate({"author": "test 404 Not Found"}, expected_status=404,
                                 expected_message="Book not found", book_id="12345678")
    ...
    def test_should_return_400_when_different_book_id_is_given_in_body(self):
        ...
        self.update_and_validate({"id": "123456", "author": "test 404 Not Found"},
                                 expected_status=400, expected_message="Updating book ID is not allowed.")
    ...
    def test_should_update_book_when_same_book_id_is_given_in_body(self):
        ...
        book = {"id": self.book_id, "author": "Update Book Author with ID in Body",
                "title": "Update Book Title with ID in Body"}
        response = self.client.put(self.client.build_url(
            f"/{self.book_id}"), json=book, headers=self.HEADERS)
